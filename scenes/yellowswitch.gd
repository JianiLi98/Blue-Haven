...
@export var target_anim: StringName = &"green"   # 撞黄→绿
...
@onready var vis: AnimatedSprite2D   = $"yellow/AnimatedSprite2D"   # 只保留黄色
...
func _apply_and_cleanup() -> void:
	...
	if is_instance_valid(player):
		...
		# 3) JumpAnim 绝对 2.0 并锁脚底（以 green_jump 为基准；若没有则退回 jump）
		var jump = (player as Node).get_node_or_null("JumpAnim") as AnimatedSprite2D
		if jump:
			var basis_anim = "jump"
			if jump.sprite_frames and jump.sprite_frames.has_animation("green_jump"):
				basis_anim = "green_jump"
